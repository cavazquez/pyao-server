...
class TestPartyRepository:
    ...
    async def test_update_member(self, party_repository, mock_redis):
        ...
        mock_redis.hset.assert_called_once_with(
            "party:1:members", "1", expected_data
        )
    ...
    async def test_save_invitation(self, party_repository, mock_redis):
        ...
        mock_redis.hset.assert_called_once_with(
            "party_invitations:2", "1", expected_data
        )
    ...
    async def test_get_user_invitations(self, party_repository, mock_redis):
        ...
        fresh_invitation = {
            ...
            "target_id": 2,
            "target_username": "Target",
            "created_at": time.time() - 10  # 10 seconds ago (fresh)
        }
        ...
        expired_invitation = {
            ...
            "target_id": 2,
            "target_username": "Target",
            "created_at": time.time() - 35  # 35 seconds ago (expired)
        }

        invitations_json = {
            "1": json.dumps(fresh_invitation),
            "2": json.dumps(expired_invitation)
        }
