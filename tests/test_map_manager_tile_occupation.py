...
def test_tile_occupation_cleared_when_npc_removed(map_manager, sample_npc):
    ...
    tile_key = (sample_npc.map_id, sample_npc.x, sample_npc.y)
    assert tile_key in map_manager._tile_occupation
    assert map_manager._tile_occupation[tile_key] == f"npc:{sample_npc.instance_id}"
    ...
    # Verificar que el tile ya NO está en _tile_occupation
    assert tile_key not in map_manager._tile_occupation
...
def test_tile_occupation_with_multiple_npcs(map_manager):
    ...
    tile1 = (npc1.map_id, npc1.x, npc1.y)
    tile2 = (npc2.map_id, npc2.x, npc2.y)
    assert tile1 in map_manager._tile_occupation
    assert tile2 in map_manager._tile_occupation
    ...
    assert tile1 not in map_manager._tile_occupation
    assert tile2 in map_manager._tile_occupation
