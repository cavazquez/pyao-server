...
class TestTaskBankDepositGold:
    ...
    async def test_deposit_gold_success(self) -> None:
        ...
        player_repo.get_gold = AsyncMock(side_effect=[500, 400])  # Antes y después
        player_repo.remove_gold = AsyncMock(return_value=True)

        bank_repo = MagicMock(spec=BankRepository)
        bank_repo.add_gold = AsyncMock(return_value=100)  # Total en banco
        ...
        # Assert
        player_repo.get_gold.assert_called_with(1)
        player_repo.remove_gold.assert_called_once_with(1, 100)
        bank_repo.add_gold.assert_called_once_with(1, 100)
        message_sender.send_update_gold.assert_called_once_with(400)
        message_sender.send_update_bank_gold.assert_called_once_with(100)
    ...
    async def test_deposit_gold_amount_zero(self) -> None:
        ...
        player_repo.remove_gold.assert_not_called()
        bank_repo.add_gold.assert_not_called()
    ...
    async def test_deposit_gold_insufficient_funds(self) -> None:
        ...
        player_repo = MagicMock(spec=PlayerRepository)
        player_repo.get_gold = AsyncMock(return_value=50)  # Solo tiene 50
    ...
    async def test_deposit_gold_without_session(self) -> None:
        ...
        player_repo.get_gold.assert_not_called()
        player_repo.remove_gold.assert_not_called()
        bank_repo.add_gold.assert_not_called()

    async def test_deposit_gold_without_dependencies(self) -> None:
