...
class WalkCommandHandler(CommandHandler):
    ...
    async def handle(self, command: Command) -> CommandResult:
        ...
        if not await self._consume_stamina(user_id):
            return CommandResult.error("No tienes suficiente stamina para moverte.")
        ...
        if not moved:
            ...
            return CommandResult.ok(
                data={"moved": False, "heading_changed": heading != current_heading}
            )
        ...
        if self.map_manager:
            ...
            if not can_move:
                ...
                if await self._can_sail_to(user_id, current_map, new_x, new_y):
                    ...
                    return CommandResult.ok(data={"moved": False, "blocked": True})
