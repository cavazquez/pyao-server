"""Handler para comando de comprar item."""
...
class CommerceBuyCommandHandler(CommandHandler):
    """Handler para comando de comprar item (solo lógica de negocio)."""
    ...
    async def handle(self, command: Command) -> CommandResult:
        """Ejecuta el comando de comprar item (solo lógica de negocio).

        Args:
            command: Comando de comprar item.

        Returns:
            Resultado de la ejecución.
        """
        if not isinstance(command, CommerceBuyCommand):
            return CommandResult.error("Comando inválido: se esperaba CommerceBuyCommand")
        ...
        logger.debug(
            "CommerceBuyCommandHandler: user_id=%d slot=%d quantity=%d", user_id, slot, quantity
        )
