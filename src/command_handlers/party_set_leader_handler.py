"""Handler para comando de transferir liderazgo de party."""
...
class PartySetLeaderCommandHandler(CommandHandler):
    """Handler para comando de transferir liderazgo de party (solo lógica de negocio)."""
    ...
    async def handle(self, command: Command) -> CommandResult:
        """Ejecuta el comando de transferir liderazgo (solo lógica de negocio).
        ...
        """
        if not isinstance(command, PartySetLeaderCommand):
            return CommandResult.error("Comando inválido: se esperaba PartySetLeaderCommand")

        user_id = command.user_id
        target_username = command.target_username

        logger.info(
            "PartySetLeaderCommandHandler: user_id=%d intenta transferir liderazgo a '%s'",
            user_id,
            target_username,
        )
        ...
        try:
            ...
            return CommandResult.error(f"Error al transferir liderazgo: {e!s}")
