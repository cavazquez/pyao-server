...
def build_clan_details_response(clan: Clan) -> bytes:
    ...
    founder_name = clan.leader_username if clan.leader_username else ""
    packet.add_unicode_string(founder_name)
    
    # FoundationDate (fecha de fundación)
    foundation_date = datetime.fromtimestamp(clan.created_at).strftime("%d/%m/%Y")
    packet.add_unicode_string(foundation_date)
    
    # Leader (nombre del líder actual)
    leader_name = clan.leader_username if clan.leader_username else ""
    packet.add_unicode_string(leader_name)
    
    # URL (sitio web del clan)
    website = clan.website if clan.website else ""
    ...
    description = clan.description if clan.description else ""
